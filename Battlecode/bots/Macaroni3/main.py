...
class Player:
    ...
    def update_map(self, ct):
        for tile in ct.get_nearby_tiles():
            self.map[tile.y][tile.x][0] = ct.get_tile_env(tile)  # Sets environment type of tile
            ...
            if ct.get_tile_building_id(tile) is not None:
                ...
                if ct.get_entity_type(ct.get_tile_building_id(tile)) in [EntityType.CONVEYOR, EntityType.ARMOURED_CONVEYOR, EntityType.SPLITTER, EntityType.GUNNER, EntityType.BREACH, EntityType.SENTINEL]:
                    ...
                    if tile not in self.map[ct.get_bridge_target(ct.get_tile_building_id(tile)).y][
                        ct.get_bridge_target(ct.get_tile_building_id(tile)).x][3]:  # Sets source of bridges ending at a tile
                        self.map[ct.get_bridge_target(ct.get_tile_building_id(tile)).y][
                            ct.get_bridge_target(ct.get_tile_building_id(tile)).x][3].append(tile)
                else:
            ...
            if ct.get_tile_env(tile) == Environment.ORE_TITANIUM and ct.get_entity_type(
                    ct.get_tile_building_id(tile)) != EntityType.HARVESTER and tile not in self.tit:
                self.tit.append(tile)
            elif tile in self.tit and (
                    ct.get_entity_type(ct.get_tile_building_id(tile)) == EntityType.HARVESTER or ct.get_team(
                    ct.get_tile_building_id(tile)) != ct.get_team()) and (self.status != 2 or not self.built_harvester[
                0]):  # Remoce from list if another bot has built harveter on it
                self.tit.remove(tile)
            elif ct.get_tile_env(tile) == Environment.ORE_AXIONITE and ct.get_entity_type(
                    ct.get_tile_building_id(tile)) != EntityType.HARVESTER and tile not in self.ax:
                self.ax.append(tile)
    ...
    def supply_connectivity(self, ct, start=None, check_back=True):  # ITERATIVE FOR EACH PATH
        ...
        connected = [
            False]  # False if not connected; True if unknown (not recorded on map); otherwise entity type of what it connects to
        next_tile = start
        ...
        while not check_back:  # Checks forward to end
            ...
            if self.map[next_tile.y][next_tile.x][1] in [EntityType.ARMOURED_CONVEYOR,
                                                         EntityType.CONVEYOR]:  # Continue in direction of conveyor
            ...
            elif self.map[next_tile.y][next_tile.x][1] in [EntityType.CORE, EntityType.BREACH, EntityType.GUNNER,
                                                           EntityType.SENTINEL]:  # Valid end point
                connected = [self.map[next_tile.y][next_tile.x][1]]
                check_back = True
            elif self.map[next_tile.y][next_tile.x][
                1] is EntityType.SPLITTER:  # Assume splitter direction is same direction of conveyor into it
                connected[0] = EntityType.SPLITTER
                left_splitter_harvester_count, left_splitter_connected = self.supply_connectivity(ct, next_tile.add(
                    self.map[next_tile.y][next_tile.x][3].rotate_left().rotate_left()), check_back)
                ...
                forward_splitter_harvester_count, forward_splitter_connected = self.supply_connectivity(ct,
                                                                                                        next_tile.add(
                                                                                                            self.map[
                                                                                                                next_tile.y][
                                                                                                                next_tile.x][
                ...
                right_splitter_harvester_count, right_splitter_connected = self.supply_connectivity(ct, next_tile.add(
                    self.map[next_tile.y][next_tile.x][3].rotate_right().rotate_right()), check_back)
    ...
    def pathfinder(self, ct, target, start=None, bridge=False, conv=False):  # Pass Position
        ...
        while not q.empty():
            ...
            if bridge:  # If bridge consider all tiles a bridge can be built to
                for i in range(7):
                    for j in range(7):
                        if (not (i == 3 and j == 3)) and ((((i - 3) ** 2) + ((j - 3) ** 2)) <= 9) and current[3].x + (
                                i - 3) >= 0 and current[3].x + (i - 3) < len(self.map[0]) and current[3].y + (
                                j - 3) >= 0 and current[3].y + (j - 3) < len(self.map) and (((self.map[current[3].y + (
                                j - 3)][current[3].x + (i - 3)][1] in [EntityType.ARMOURED_CONVEYOR, EntityType.BRIDGE,
                                                                       EntityType.CONVEYOR, EntityType.MARKER,
                                                                       EntityType.ROAD, EntityType.CORE] and self.map[
                                                                                                  current[3].y + (
                                                                                                          j - 3)][
                                                                                                  current[3].x + (
                                                                                                          i - 3)][
                                                                                                     ...
                                                                                                     self.map[current[
                                                                                                         ...
                                                                                                         current[
                                                                                                             3].x + (
                                                                                                                 i - 3)][
                                                                                                         0] not in [
                                                                                                         Environment.WALL]))):  # (not (self.map[current[3].y + (j-3)][current[3].x + (i-3)][4] in [EntityType.BUILDER_BOT] and ct.get_position().distance_squared(current[3]) <= 9)) and
                ...
                for i in range(3):
                    for j in range(3):
                        if (not (abs(i - 1) == abs(j - 1))) and current[3].x + (i - 1) >= 0 and current[3].x + (
                                i - 1) < len(self.map[0]) and current[3].y + (j - 1) >= 0 and current[3].y + (
                                j - 1) < len(self.map) and (((self.map[current[3].y + (j - 1)][current[3].x + (i - 1)][
                                                                  1] in [EntityType.ARMOURED_CONVEYOR,
                                                                         EntityType.BRIDGE, EntityType.CONVEYOR,
                                                                         EntityType.MARKER, EntityType.ROAD,
                                                                         EntityType.CORE] and
                                                              self.map[current[3].y + (j - 1)][current[3].x + (i - 1)][
                                                                  2] == ct.get_team()) or (
                                                                     self.map[current[3].y + (j - 1)][
                                                                         current[3].x + (i - 1)][1] == None and
                                                                     self.map[current[3].y + (j - 1)][
                                                                         current[3].x + (i - 1)][0] not in [
                                                                         Environment.WALL]))):  # (not (self.map[current[3].y + (j-1)][current[3].x + (i-1)][4] in [EntityType.BUILDER_BOT] and ct.get_position().distance_squared(current[3]) <= 1) and
                ...
                for i in range(3):
                    for j in range(3):
                        if (not (i == 1 and j == 1)) and current[3].x + (i - 1) >= 0 and current[3].x + (i - 1) < len(
                                self.map[0]) and current[3].y + (j - 1) >= 0 and current[3].y + (j - 1) < len(
                                self.map) and ((not (self.map[current[3].y + (j - 1)][current[3].x + (i - 1)][4] in [
                            EntityType.BUILDER_BOT] and ct.get_position().distance_squared(current[3]) <= 2)) and (
                                                       self.map[current[3].y + (j - 1)][current[3].x + (i - 1)][1] in [
                                                   EntityType.ARMOURED_CONVEYOR, EntityType.BRIDGE, EntityType.CONVEYOR,
                                                   EntityType.MARKER, EntityType.ROAD] or (
                                                               self.map[current[3].y + (j - 1)][current[3].x + (i - 1)][
                                                                   1] == EntityType.CORE and
                                                               self.map[current[3].y + (j - 1)][current[3].x + (i - 1)][
                                                                   2] == ct.get_team()) or (
                                                               self.map[current[3].y + (j - 1)][current[3].x + (i - 1)][
                                                                   1] == None and
            ...
            for tile in check_tiles:
                ...
                # counter += 1
                if self.map[tile[1]][tile[0]][
                    1] == None:  # Prefer not to move over non-passable spaces (to save resources building extra paths)
                    moveTile += 1
                if bridge:
                    dist = dist - current[3].distance_squared(tile_pos)  # Prefer to build longest bridge
                else:
                    dist = current[3].distance_squared(
                        tile_pos)  # Prefer to move in straight lines (as I think is more valuable for information)
                ...
                if bridge:
                    ...
                    new_cost = cost_so_far[current[
                        3]] + 1  # Each move costs one move cooldown whether straight or diagonal for general movement
                if tile_pos not in cost_so_far or new_cost < cost_so_far[
                    tile_pos]:  # Considers tile if not considered before or new path gets to it quicker
    ...
    def explore(self, ct, target=None):
        ...
        for tile in ct.get_nearby_tiles():  # Find the closest passable tile to target in vision
            if (ct.get_position() != tile and tile.distance_squared(target) < closest_tile.distance_squared(target)
                    and ct.get_tile_builder_bot_id(tile) == None
                    and ((self.map[tile.y][tile.x][1] == EntityType.CORE and self.map[tile.y][tile.x][2] == ct.get_team()) or
                            self.map[tile.y][tile.x][1] in [EntityType.ARMOURED_CONVEYOR, EntityType.BRIDGE, EntityType.CONVEYOR, EntityType.MARKER, EntityType.ROAD, None])):
                closest_tile = tile
        if closest_tile != Position(1000, 1000):
            ...
            if len(path_explore) == 0:  # If there is no moveable path to target
                ...
                if ct.get_move_cooldown() == 0:
                    ct.draw_indicator_line(ct.get_position(), self.target, 0, 0, 0)
                    print("STUCK")
            else:
                ...
                if ct.can_move(ct.get_position().direction_to(path_explore[1])):
                    ...
                    if ct.get_tile_builder_bot_id(path_explore[1]) != None:
                        ...
                        ct.draw_indicator_line(ct.get_position(),
                                               ct.get_position().add(ct.get_position().direction_to(path_explore[1])),
                                               0, 255, 0)
                        # ct.resign()
                        # Ran out of money
    ...
    def harvest_ore(self, ct, ore):
        if not self.built_harvester[0]:  # If have not built harvester, must move towards it and build
            ...
            if len(path_move_harvester) == 0:  # If another bot builds a harvester on ore on same turn then raises error (should change in map to remove ores from list if it contains a harvester)
                ct.draw_indicator_line(ct.get_position(), ore, 255, 0, 0)
                ct.resign()

            came_from_first_conv, cost_first_conv, best_tile_first_conv = self.pathfinder(ct, ore,
                                                                                          conv=True)  # Conveyor build path from current location to ore
            ...
            if len(path_first_conv) == 0:
                ...
                came_from_first_conv, cost_first_conv, best_tile_first_conv = self.pathfinder(ct, ore,
                                                                                              bridge=True)  # Bridge build path from current location to ore    I THINK WILL CAUSE SOME ERRORS
                path_first_conv = self.reconstruct_path(came_from_first_conv, best_tile_first_conv)
            if len(path_first_conv) < 2:
                self.tit.remove(ore)  # Strange case so just give up to orevent error
                return
            if self.map[path_first_conv[-2].y][path_first_conv[-2].x][1] == EntityType.ROAD and \
                    self.map[path_first_conv[-2].y][path_first_conv[-2].x][2] == ct.get_team() and ct.can_destroy(
                    path_first_conv[-2]) and ct.get_position().distance_squared(ore) <= 5:
                ct.destroy(path_first_conv[-2])
            elif self.map[path_first_conv[-2].y][path_first_conv[-2].x][
                2] != ct.get_team() and ct.get_position().distance_squared(ore) <= 5:
                self.tit.remove(
                    ore)  # If other team have built a building where you want to build conveyor just forget about it
            elif ct.get_position() != path_first_conv[-2] and self.map[path_first_conv[-2].y][path_first_conv[-2].x][
                1] in [EntityType.CONVEYOR] and self.map[path_first_conv[-2].y][path_first_conv[-2].x][
                2] == ct.get_team() and ct.get_position().distance_squared(
                    ore) <= 5:  # May be other builder bot sitting and waiting for money to build harvester    ct.get_entity_type(ct.get_tile_builder_bot_id(path_first_conv[-2])) == EntityType.BUILDER_BOT
                self.tit.remove(ore)
            if ct.can_build_conveyor(path_first_conv[-2], Direction.NORTH) and ct.get_position().distance_squared(
                    ore) <= 5:  # Check if can build conveyor next to ore
                came_from_harvester_core, cost_from_harvester_core, best_tile_harvester_core = self.pathfinder(ct,
                                                                                                               self.core_pos,
                                                                                                               path_first_conv[
                                                                                                                   -2],
                                                                                                               conv=True)
                came_from_harvester_conn, cost_from_harvester_conn, best_tile_harvester_conn = self.pathfinder(ct,
                                                                                                               self.closest_conn_to_core[
                                                                                                                   0],
                                                                                                               path_first_conv[
                                                                                                                   -2],
                                                                                                               conv=True)
                if cost_from_harvester_core[best_tile_harvester_core] <= cost_from_harvester_conn[
                    best_tile_harvester_conn]:  # Must choose to build conveyors back to core or closest conveyor as stored
                    path_from_harvester = self.reconstruct_path(came_from_harvester_core, best_tile_harvester_core)
                    if len(path_from_harvester) > 5 + (
                    path_first_conv[-2].distance_squared(best_tile_harvester_core)) ** (
                            1 / 2):  # Uses bridge if cost of a bridge path around obstacle is less than conveyor path (based on scaling factor)
                        came_from_harvester, cost_from_harvester, best_tile_harvester_core = self.pathfinder(ct,
                                                                                                             self.core_pos,
                                                                                                             path_first_conv[
                                                                                                                 -2],
                                                                                                             bridge=True)
                        ...
                        if len(path_from_harvester) > 1:
                            came_from_harvester_conv_check, cost_from_harvester_conv_check, best_tile_harvester_core_conv_check = self.pathfinder(
                                ct, path_from_harvester[1], path_first_conv[-2], conv=True)
                            if best_tile_harvester_core_conv_check == path_from_harvester[1] and \
                                    cost_from_harvester_conv_check[best_tile_harvester_core_conv_check] == (
                                    abs(path_from_harvester[1].x - path_first_conv[-2].x) + abs(
                                    path_from_harvester[1].y - path_first_conv[
                                        -2].y)):  # If can build conveyors between these points directly
                                came_from_harvester = came_from_harvester_conv_check
                                cost_from_harvester = cost_from_harvester_conv_check
                                path_from_harvester = self.reconstruct_path(came_from_harvester,
                                                                            best_tile_harvester_core_conv_check)
                    else:
                    ...
                    if len(path_from_harvester) > 5 + (
                    path_first_conv[-2].distance_squared(best_tile_harvester_conn)) ** (1 / 2):
                        came_from_harvester, cost_from_harvester, best_tile_harvester_conn = self.pathfinder(ct,
                                                                                                             self.closest_conn_to_core[
                                                                                                                 0],
                                                                                                             path_first_conv[
                                                                                                                 -2],
                        ...
                        if len(path_from_harvester) > 1:
                            came_from_harvester_conv_check, cost_from_harvester_conv_check, best_tile_harvester_conn_conv_check = self.pathfinder(
                                ct, path_from_harvester[1], path_first_conv[-2], conv=True)
                            if best_tile_harvester_conn_conv_check == path_from_harvester[1] and \
                                    cost_from_harvester_conv_check[best_tile_harvester_conn_conv_check] == (
                                    abs(path_from_harvester[1].x - path_first_conv[-2].x) + abs(
                                    path_from_harvester[1].y - path_first_conv[
                                        -2].y)):  # If can build conveyors between these points directly
                                came_from_harvester = came_from_harvester_conv_check
                                cost_from_harvester = cost_from_harvester_conv_check
                                path_from_harvester = self.reconstruct_path(came_from_harvester,
                                                                            best_tile_harvester_conn_conv_check)
                    else:
                ...
                if len(path_from_harvester) == 0:
                    ...
                    if path_from_harvester[0].distance_squared(path_from_harvester[1]) == 1 and ct.can_build_conveyor(
                            path_from_harvester[0], path_from_harvester[0].direction_to(path_from_harvester[1])):
                        self.closest_conn_to_core[1] = True
                        ct.build_conveyor(path_from_harvester[0],
                                          path_from_harvester[0].direction_to(path_from_harvester[1]))
                    ...
                    elif self.map[path_from_harvester[0].y][path_from_harvester[0].x][1] in [
                        EntityType.ARMOURED_CONVEYOR, EntityType.BRIDGE, EntityType.CONVEYOR, EntityType.SPLITTER] and \
                            self.map[path_from_harvester[0].y][path_from_harvester[0].x][
                                2] == ct.get_team():  # If already a friendly transport mechanism in space then move on
                        self.closest_conn_to_core[1] = True
                    else:
                        ct.draw_indicator_dot(path_from_harvester[0], 0, 0, 0)
                        ct.draw_indicator_line(path_from_harvester[0], path_from_harvester[1], 255, 0, 255)
                        # ct.resign()    # Probbaly ran out of money
                    if ct.can_move(ct.get_position().direction_to(
                            path_from_harvester[0])):  # Moves on to built conveyor or bridge
                        ct.move(ct.get_position().direction_to(path_from_harvester[0]))
                    else:  # Ran out of money
                        ct.draw_indicator_line(ct.get_position(), ct.get_position().add(
                            ct.get_position().direction_to(path_from_harvester[0])), 0, 0, 255)
                        # ct.resign()
            elif self.map[path_first_conv[-1].y][path_first_conv[-1].x][1] == EntityType.ROAD and \
                    self.map[path_first_conv[-1].y][path_first_conv[-1].x][2] == ct.get_team() and ct.can_destroy(
                    path_first_conv[-1]) and ct.get_position().distance_squared(ore) <= 1:
                ct.destroy(path_first_conv[-1])  # Destroy road built over ore
            elif ct.can_build_harvester(ore) and ore in self.tit and self.built_harvester[
                1] != False:  # If can build harvester, build it
                ...
                self.closest_conn_to_core[
                    1] = True  # Ensures this happens in case that does not need to build any conveyors tp connect harvester
                ct.build_harvester(ore)
                if self.map[ct.get_position().y][ct.get_position().x][1] in [EntityType.CONVEYOR,
                                                                             EntityType.ARMOURED_CONVEYOR] and \
                        self.map[ct.get_position().y][ct.get_position().x][2] == ct.get_team():
            ...
            if self.built_harvester[1] != False:  # If bridge built
                if ct.get_position().distance_squared(
                        self.built_harvester[1]) <= 2:  # If directly next to position it needs to move to
                    came_from_built_harvester_core, cost_from_built_harvester_core, best_tile_built_harvester_core = self.pathfinder(
                        ct, self.core_pos, self.built_harvester[1], conv=True)
                    came_from_built_harvester_conn, cost_from_built_harvester_conn, best_tile_built_harvester_conn = self.pathfinder(
                        ct, self.closest_conn_to_core[0], self.built_harvester[1], conv=True)
                    if cost_from_built_harvester_core[best_tile_built_harvester_core] <= cost_from_built_harvester_conn[
                        best_tile_built_harvester_conn]:  # Choose closest between core and stored closest cnveyor
                        path_from_built_harvester = self.reconstruct_path(came_from_built_harvester_core,
                                                                          best_tile_built_harvester_core)
                        if len(path_from_built_harvester) > 5 + (
                        self.built_harvester[1].distance_squared(best_tile_built_harvester_core)) ** (
                                1 / 2):  # Uses bridge if cost of a bridge path around obstacle is less than conveyor path (based on scaling factor):
                            came_from_built_harvester, cost_from_built_harvester, best_tile_built_harvester_core = self.pathfinder(
                                ct, self.core_pos, self.built_harvester[1], bridge=True)
                            path_from_built_harvester = self.reconstruct_path(came_from_built_harvester,
                                                                              best_tile_built_harvester_core)
                            if len(path_from_built_harvester) > 1:
                                came_from_built_harvester_conv_check, cost_from_built_harvester_conv_check, best_tile_built_harvester_core_conv_check = self.pathfinder(
                                    ct, path_from_built_harvester[1], self.built_harvester[1], conv=True)
                                if best_tile_built_harvester_core_conv_check == path_from_built_harvester[1] and \
                                        cost_from_built_harvester_conv_check[
                                            best_tile_built_harvester_core_conv_check] == (
                                        abs(path_from_built_harvester[1].x - self.built_harvester[1].x) + abs(
                                        path_from_built_harvester[1].y - self.built_harvester[
                                            1].y)):  # If can build conveyors between these points directly
                        ...
                        path_from_built_harvester = self.reconstruct_path(came_from_built_harvester_conn,
                                                                          best_tile_built_harvester_conn)
                        if len(path_from_built_harvester) > 5 + (
                        self.built_harvester[1].distance_squared(best_tile_built_harvester_conn)) ** (1 / 2):
                            came_from_built_harvester, cost_from_built_harvester, best_tile_built_harvester_conn = self.pathfinder(
                                ct, self.closest_conn_to_core[0], self.built_harvester[1], bridge=True)
                            path_from_built_harvester = self.reconstruct_path(came_from_built_harvester,
                                                                              best_tile_built_harvester_conn)
                            if len(path_from_built_harvester) > 1:
                                came_from_built_harvester_conv_check, cost_from_built_harvester_conv_check, best_tile_built_harvester_conn_conv_check = self.pathfinder(
                                    ct, path_from_built_harvester[1], self.built_harvester[1], conv=True)
                                if best_tile_built_harvester_conn_conv_check == path_from_built_harvester[1] and \
                                        cost_from_built_harvester_conv_check[
                                            best_tile_built_harvester_conn_conv_check] == (
                                        abs(path_from_built_harvester[1].x - self.built_harvester[1].x) + abs(
                                        path_from_built_harvester[1].y - self.built_harvester[
                                            1].y)):  # If can build conveyors between these points directly
                                    came_from_built_harvester = came_from_built_harvester_conv_check
                    ...
                    if len(path_from_built_harvester) == 0:
                        ...
                        ct.resign()
                    elif len(path_from_built_harvester) == 1 or (
                            self.map[path_from_built_harvester[0].y][path_from_built_harvester[0].x][1] in [
                        EntityType.CORE, EntityType.ARMOURED_CONVEYOR, EntityType.BRIDGE, EntityType.CONVEYOR,
                        EntityType.SPLITTER] and
                            self.map[path_from_built_harvester[0].y][path_from_built_harvester[0].x][
                                2] == ct.get_team()):  # Check for being at end of path
                        self.built_harvester[0] = False
                        self.closest_conn_to_core[1] = False
                        if len(path_from_built_harvester) > 1:
                            ct.draw_indicator_line(path_from_built_harvester[0], path_from_built_harvester[1], 255, 0,
                                                   255)
                        if len(path_from_built_harvester) > 0:
                            ct.draw_indicator_line(path_from_built_harvester[0], self.core_pos, 0, 255, 255)
                            ct.draw_indicator_line(path_from_built_harvester[0], self.closest_conn_to_core[0], 0, 255,
                                                   255)
                        self.tit.remove(ore)  # Remove ore from build queue
                    else:
                        if \
                        self.map[ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[0])).y][
                            ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[0])).x][
                            1] == EntityType.ROAD and self.map[
                            ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[0])).y][
                            ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[0])).x][
                            2] == ct.get_team() and ct.can_destroy(ct.get_position().add(ct.get_position().direction_to(
                                path_from_built_harvester[0]))):  # If a friendly road is on path, destroy it
                            ct.destroy(
                                ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[0])))
                        if path_from_built_harvester[0].distance_squared(
                                path_from_built_harvester[1]) == 1 and ct.can_build_conveyor(
                                path_from_built_harvester[0], path_from_built_harvester[0].direction_to(
                                        path_from_built_harvester[1])):  # Build conveyor and move on to it
                            ct.build_conveyor(path_from_built_harvester[0],
                                              path_from_built_harvester[0].direction_to(path_from_built_harvester[1]))
                            if ct.can_move(ct.get_position().direction_to(path_from_built_harvester[0])):
                                ...
                                self.target = ct.get_position().add(
                                    ct.get_position().direction_to(path_from_built_harvester[0]))
                            ...
                            ct.draw_indicator_line(path_from_built_harvester[0], path_from_built_harvester[1], 255, 255,
                                                   255)
                ...
                came_from_built_harvester_conn, cost_from_built_harvester_conn, best_tile_built_harvester_conn = self.pathfinder(
                    ct, self.closest_conn_to_core[0], conv=True)
                ...
                if cost_from_built_harvester_core[best_tile_built_harvester_core] <= cost_from_built_harvester_conn[
                    ...
                    path_from_built_harvester = self.reconstruct_path(came_from_built_harvester_core,
                                                                      best_tile_built_harvester_core)
                    if len(path_from_built_harvester) > 5 + (
                    ct.get_position().distance_squared(best_tile_built_harvester_core)) ** (1 / 2) or (
                            len(path_from_built_harvester) == 2 and path_from_built_harvester[1] not in [
                        EntityType.ARMOURED_CONVEYOR, EntityType.CONVEYOR, EntityType.CORE, EntityType.BRIDGE,
                        EntityType.SPLITTER]) or (
                        ...
                        if ct.get_tile_building_id(ct.get_position()) != None and ct.get_entity_type(
                                ct.get_tile_building_id(ct.get_position().add(ct.get_direction(
                                        ct.get_tile_building_id(ct.get_position()))))) != EntityType.HARVESTER:
                            came_from_built_harvester, cost_from_built_harvester, best_tile_built_harvester_core = self.pathfinder(
                                ct, self.core_pos,
                                ct.get_position().add(ct.get_direction(ct.get_tile_building_id(ct.get_position()))),
                                bridge=True)
                        else:
                            came_from_built_harvester, cost_from_built_harvester, best_tile_built_harvester_core = self.pathfinder(
                                ct, self.core_pos, bridge=True)
                        path_from_built_harvester = self.reconstruct_path(came_from_built_harvester,
                                                                          best_tile_built_harvester_core)
                        ...
                        if len(path_from_built_harvester) > 1:
                            came_from_built_harvester_conv_check, cost_from_built_harvester_conv_check, best_tile_built_harvester_core_conv_check = self.pathfinder(
                                ct, path_from_built_harvester[1], conv=True)
                            if best_tile_built_harvester_core_conv_check == path_from_built_harvester[1] and \
                                    cost_from_built_harvester_conv_check[best_tile_built_harvester_core_conv_check] == (
                                    abs(path_from_built_harvester[1].x - ct.get_position().x) + abs(
                                    path_from_built_harvester[
                                        1].y - ct.get_position().y)):  # If can build conveyors between these points directly
                                came_from_built_harvester = came_from_built_harvester_conv_check
                                cost_from_built_harvester = cost_from_built_harvester_conv_check
                                path_from_built_harvester = self.reconstruct_path(came_from_built_harvester,
                                                                                  best_tile_built_harvester_core_conv_check)
                    ...
                    path_from_built_harvester = self.reconstruct_path(came_from_built_harvester_conn,
                                                                      best_tile_built_harvester_conn)
                    if len(path_from_built_harvester) > 5 + ct.get_position().distance_squared(
                            best_tile_built_harvester_conn) or (
                            len(path_from_built_harvester) == 2 and path_from_built_harvester[1] not in [
                        EntityType.ARMOURED_CONVEYOR, EntityType.CONVEYOR, EntityType.CORE, EntityType.BRIDGE,
                        EntityType.SPLITTER]) or (
                            len(path_from_built_harvester) == 1 and best_tile_built_harvester_conn !=
                            self.closest_conn_to_core[0]):
                        if ct.get_tile_building_id(ct.get_position()) != None and ct.get_entity_type(
                                ct.get_tile_building_id(ct.get_position().add(ct.get_direction(
                                        ct.get_tile_building_id(ct.get_position()))))) != EntityType.HARVESTER:
                            came_from_built_harvester, cost_from_built_harvester, best_tile_built_harvester_conn = self.pathfinder(
                                ct, self.closest_conn_to_core[0],
                                ct.get_position().add(ct.get_direction(ct.get_tile_building_id(ct.get_position()))),
                                bridge=True)
                            ct.draw_indicator_line(self.closest_conn_to_core[0], ct.get_position().add(
                                ct.get_direction(ct.get_tile_building_id(ct.get_position()))), 255, 0, 255)
                        else:
                            came_from_built_harvester, cost_from_built_harvester, best_tile_built_harvester_conn = self.pathfinder(
                                ct, self.closest_conn_to_core[0], bridge=True)
                        path_from_built_harvester = self.reconstruct_path(came_from_built_harvester,
                                                                          best_tile_built_harvester_conn)
                        if len(path_from_built_harvester) > 1 and ct.get_position() != path_from_built_harvester[1]:
                            ct.draw_indicator_line(path_from_built_harvester[0], path_from_built_harvester[1], 0, 255,
                                                   255)
                            ...
                            came_from_built_harvester_conv_check, cost_from_built_harvester_conv_check, best_tile_built_harvester_conn_conv_check = self.pathfinder(
                                ct, path_from_built_harvester[1], conv=True)
                            if best_tile_built_harvester_conn_conv_check == path_from_built_harvester[1] and \
                                    cost_from_built_harvester_conv_check[best_tile_built_harvester_conn_conv_check] == (
                                    abs(path_from_built_harvester[1].x - ct.get_position().x) + abs(
                                    path_from_built_harvester[
                                        1].y - ct.get_position().y)):  # If can build conveyors between these points directly
                                came_from_built_harvester = came_from_built_harvester_conv_check
                ...
                if len(path_from_built_harvester) == 0:
                    ...
                    ct.resign()
                        ...
                        ct.get_position().distance_squared(path_from_built_harvester[1]) == 1 and
                        self.map[path_from_built_harvester[1].y][path_from_built_harvester[1].x][1] in [EntityType.CORE,
                                                                                                        EntityType.ARMOURED_CONVEYOR,
                                                                                                        EntityType.BRIDGE,
                                                                                                        EntityType.CONVEYOR,
                                                                                                        EntityType.SPLITTER] and
                        self.map[path_from_built_harvester[1].y][path_from_built_harvester[1].x][
                            2] == ct.get_team() and not (
                        self.map[path_from_built_harvester[1].y][path_from_built_harvester[1].x][1] in [
                    EntityType.CONVEYOR, EntityType.ARMOURED_CONVEYOR] and
                        self.map[path_from_built_harvester[1].y][path_from_built_harvester[1].x][
                            3] == ct.get_position().direction_to(path_from_built_harvester[1]).opposite())) or ((
                        ct.get_position().distance_squared(path_from_built_harvester[0]) == 1 and
                        self.map[path_from_built_harvester[0].y][path_from_built_harvester[0].x][1] in [EntityType.CORE,
                                                                                                        EntityType.ARMOURED_CONVEYOR,
                                                                                                        EntityType.BRIDGE,
                                                                                                        EntityType.CONVEYOR,
                                                                                                        EntityType.SPLITTER] and
                        self.map[path_from_built_harvester[0].y][path_from_built_harvester[0].x][
                            2] == ct.get_team())):  # Check for being at end of path MAY NEED To ADD TO FOR BRIDGE STUFF
                    self.built_harvester[0] = False
                    self.closest_conn_to_core[1] = False
                    ...
                    if ct.get_position().distance_squared(path_from_built_harvester[1]) == 1 and (self.map[
                                                                                                      ct.get_position().add(
                                                                                                              ct.get_position().direction_to(
                                                                                                                      path_from_built_harvester[
                                                                                                                          1])).y][
                                                                                                      ct.get_position().add(
                                                                                                              ct.get_position().direction_to(
                                                                                                  ...
                                                                                                  self.map[
                                                                                                      ...
                                                                                                      ct.get_position().add(
                                                                                                              ct.get_position().direction_to(
                                                                                                                      path_from_built_harvester[
                                                                                                                          1])).x][
                                                                                                      3] == ct.get_position().direction_to(
                                    path_from_built_harvester[1]).opposite()) and ct.can_destroy(ct.get_position().add(
                            ct.get_position().direction_to(
                    ...
                    elif ct.get_position().distance_squared(path_from_built_harvester[0]) == 1 and self.map[
                        ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[0])).y][
                        ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[0])).x][
                        1] == EntityType.ROAD and self.map[
                        ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[0])).y][
                    ...
                    self.map[ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[1])).y][
                        ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[1])).x][
                        1] == EntityType.HARVESTER and self.map[
                        ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[1])).y][
                        ct.get_position().add(ct.get_position().direction_to(path_from_built_harvester[1])).x][
                        2] == ct.get_team() and path_from_built_harvester[0] == ct.get_position() and \
                            self.map[ct.get_position().y][ct.get_position().x][1] in [EntityType.ARMOURED_CONVEYOR,
                                                                                      EntityType.CONVEYOR,
                                                                                      EntityType.ROAD,
                                                                                      EntityType.MARKER] and \
                            self.map[ct.get_position().y][ct.get_position().x][2] == ct.get_team() and ct.can_destroy(
                            ct.get_position()):
                        ct.destroy(ct.get_position())
                    if ct.get_tile_building_id(ct.get_position()) != None and ((ct.get_position().distance_squared(
                            path_from_built_harvester[1]) == 1 and ct.get_direction(
                            ct.get_tile_building_id(ct.get_position())) != ct.get_position().direction_to(
                            path_from_built_harvester[1])) or (ct.get_position().distance_squared(
                            path_from_built_harvester[0]) == 1 and ct.get_direction(
                            ct.get_tile_building_id(ct.get_position())) != ct.get_position().direction_to(
                        ...
                        0] and ct.get_position().distance_squared(path_from_built_harvester[1]) > 1)) and \
                            self.map[ct.get_position().y][ct.get_position().x][1] in [EntityType.CONVEYOR,
                                                                                      EntityType.ARMOURED_CONVEYOR] and \
                            self.map[ct.get_position().y][ct.get_position().x][2] == ct.get_team() and ct.can_destroy(
                            ct.get_position()):  # If finds new path that is more optimal but current conveyor faces in the wrong direction must rebuild
                        ct.destroy(ct.get_position())
                    if ct.get_position().distance_squared(path_from_built_harvester[1]) == 1 and ct.can_build_conveyor(
                            ct.get_position(), ct.get_position().direction_to(path_from_built_harvester[1])):
                        ct.build_conveyor(ct.get_position(),
                                          ct.get_position().direction_to(path_from_built_harvester[1]))
                        ...
                        if path_from_built_harvester[1] in self.tit:
                            self.tit.remove(
                                path_from_built_harvester[1])  # Can remove this as still connecting other ore
                            ...
                            path_from_built_harvester[2]) == 1 and ct.can_build_conveyor(path_from_built_harvester[1],
                                                                                         path_from_built_harvester[
                                                                                             1].direction_to(
                                                                                                 path_from_built_harvester[
                                                                                                     2])) and \
                            self.map[path_from_built_harvester[1].y][path_from_built_harvester[1].x][0] not in [
                        Environment.ORE_AXIONITE, Environment.ORE_TITANIUM]:  # Build conveyor and move on to it
                        ct.build_conveyor(path_from_built_harvester[1],
                                          path_from_built_harvester[1].direction_to(path_from_built_harvester[2]))
                        ...
                        ct.draw_indicator_dot(path_from_built_harvester[0], 255, 0, 255)
                        if len(path_from_built_harvester) > 1:
                            ct.draw_indicator_line(path_from_built_harvester[0], path_from_built_harvester[0].add(
                                path_from_built_harvester[0].direction_to(path_from_built_harvester[1])), 255, 255, 255)
                        if len(path_from_built_harvester) > 2:
                            ct.draw_indicator_line(path_from_built_harvester[1], path_from_built_harvester[1].add(
                                path_from_built_harvester[1].direction_to(path_from_built_harvester[2])), 255, 255, 255)
                        # if path_from_built_harvester[0].distance_squared(path_from_built_harvester[1]) > 1:
                        # self.built_harvester[1] = path_from_built_harvester[0]
                        # ct.resign()
    ...
    def report_enemy_core_location(self,ct):
        ...
        for i in DIRECTIONS:
            if ct.get_position().distance_squared(self.core_pos) <= 25:
    ...
    def exploring(self, ct):
        if len(self.tit) != 0:  # Check if can mine ore at location
            ...
            iteration = 0
            while closest_corner == Position(1000, 1000) and iteration < (Position(0, 0).distance_squared(
                    Position(int(ct.get_map_width() / 2),
                             int(ct.get_map_height() / 2)))) / 7:  # Ends if new position to explore is found or is at centre
